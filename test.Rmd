...
interests, and concerns of the scientific staff at the department of civil, environmental, 
and geomatic engineering (D-BAUG) at ETH Zurich. This survey will be conducted
...
# Sample composition
First, the background of the survey's participants are described by the following factors: age, gender, citizenship, and their salary levels as a doctoral student.
## Age
```{r 2B-age}
#age distribution
#table(finishers$Q4)
...
ggplot(finishers_long, aes(x = Value, fill = salary)) +
    ...
    geom_vline(aes(xintercept = Mean), linetype = "dashed", color = "red", size = 0.3) +
    facet_wrap(~ Variable, scales = "free_y",  labeller = label_wrap_gen(width = 25)) + 
...
***
## Teaching level relative to other team members
```{r teaching-more}
...
- 32% of doctoral students had expected more direct supervision by their supervising professor.
Based on the individual comments on what has been going well and what not, the following can be summarized:
